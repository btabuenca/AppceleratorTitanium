...
@implementation ouClicker4iPodObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ouclicker4ipod);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any ouClicker4iPod patches in JS before a context is loaded... nice for 
...
{
	// create ouClicker4iPod global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ouclicker4ipod = [[ouClicker4iPodObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ouclicker4ipod];
	
	NSString *_ouclicker4ipodNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ouclicker4ipodNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ouclicker4ipod valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.ouclicker4ipod.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
