...
@implementation StropheXmppClientObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_strophexmppclient);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any StropheXmppClient patches in JS before a context is loaded... nice for 
...
{
	// create StropheXmppClient global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_strophexmppclient = [[StropheXmppClientObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_strophexmppclient];
	
	NSString *_strophexmppclientNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _strophexmppclientNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_strophexmppclient valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.strophexmppclient.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
